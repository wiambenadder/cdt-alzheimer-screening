...
|---|------|--------|-------------------|
| 1 | In noteboks/01_data_exploration.ipynb and noteboks/05_error_analysis.ipynb. We test three architectures matching the paper's comparison: a CNN baseline (VGG16 in place of the paper's ResNet101, comparable era and ImageNet accuracy), EfficientNet-B0 (same as paper), and ViT-B/16 (same as paper). Results on our 8,938-clock test set: EfficientNet-B0 quadratic kappa = 0.747 (paper: 0.73, we outperform by 0.017), ViT-B/16 quadratic kappa = 0.808 (paper: 0.81, within 0.002). Ablation best configuration (unfrozen backbone and augmentation) reaches kappa = 0.812, slightly exceeding the paper. Our evaluation is strictly harder because we use participant-disjoint splits and 2.4x more training data than the paper. Comparison table in README. Full results in docs/results/all_results.csv. |
