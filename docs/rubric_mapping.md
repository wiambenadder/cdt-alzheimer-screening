...
|---|------|--------|-------------------|
| 1 | Reproduced quantitative results from published research paper | 10 | In notebooks/03_finetuning.ipynb and notebooks/05_error_analysis.ipynb, we test three architectures matching the paper's comparison: a CNN baseline (VGG16 in place of the paper's ResNet101, comparable era and ImageNet accuracy), EfficientNet-B0 (same as paper), and ViT-B/16 (same as paper). Results on our 8,938-clock test set: EfficientNet-B0 quadratic kappa = 0.747 (paper: 0.73, we outperform by 0.017), ViT-B/16 quadratic kappa = 0.808 (paper: 0.81, within 0.002). Ablation best configuration (unfrozen backbone and augmentation) reaches kappa = 0.812, slightly exceeding the paper. Our evaluation is strictly harder because we use participant-disjoint splits and 2.4x more training data than the paper. Comparison table in README. Full results in results/all_results.csv. 
...
| 5 | Error analysis with visualization and failure case discussion | 7 | NB05: confusion matrices saved to results/vit_confusion_matrix_normalized.png. Top-12 errors in results/errors/ (individual misclassified clock images, 12 examples). Most errors are adjacent-class confusions (ordinal distance 1). Class 2 is hardest at F1=0.60. Per-class report in NB05 output. |
| 6 | Interpretable model design or explainability analysis | 7 | NB05: patch occlusion maps for all 6 CDT classes in results/vit_attribution_maps.png. For scores 4 and 5 the model attends to the clock face and hands. src/gradcam.py vit_occlusion_map() implements the patch masking loop. Inference time 2.3 ms per image also measured in NB05. |
| 7 | Ablation study varying at least 2 independent design choices | 7 | NB04: 2x2 factorial on ViT-B/16. Factor 1 is backbone frozen vs unfrozen. Factor 2 is augmentation off vs on. Results: frozen/noaug kappa=0.667, frozen/aug kappa=0.640, unfrozen/noaug kappa=0.799, unfrozen/aug kappa=0.812. Summary table in README and results/all_results.csv. |
| 8 | Compared multiple architectures quantitatively | 7 | NB03: VGG16 kappa=0.781, EfficientNet-B0 kappa=0.747, ViT-B/16 kappa=0.808. Same split, same loss, same epoch budget. Full metrics in results/all_results.csv and README. |
...
| 13 | Systematic hyperparameter tuning (at least 3 configs) | 5 | NB03: 3 architecture configs compared on the same validation split. VGG16 kappa=0.781, EfficientNet-B0 kappa=0.747, ViT-B/16 kappa=0.808. Results in results/all_results.csv. |
...
| 15 | Training curves tracked and visualized | 3 | src/train.py SummaryWriter writes loss, accuracy, and learning rate to TensorBoard at every epoch. Logs written to results/<run>/tb/. |
